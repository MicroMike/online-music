...
// MongoDB Connection
mongoose.connect(process.env.MONGODB_URI || 'mongodb+srv://root:123456Ff@cluster0.zix0h.mongodb.net/Music?retryWrites=true&w=majority', (error) => {
	if (error) {
...
const checkRun = () => {
	Object.values(checkRunArray).forEach(arr => {
		...
		const tooManyLoad = Object.values(streams).filter(s => s.parentId[0] === parentId[0] && s.infos && s.infos.other).length > 0
