...
io.on('connection', client => {
  ...
  client.on('parent', ({ parentId, s }) => {
    ...
    loopInter = setInterval(() => {
      const RUN_WAIT_PAGE = Object.values(streams).filter(s => s.parentId === parentId && s.infos && (s.infos.freeze || s.infos.time && String(s.infos.time).match(/RUN|WAIT_PAGE/))).length
